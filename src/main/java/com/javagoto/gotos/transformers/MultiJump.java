...
public class MultiJump {
    ...
    static void makeMultiJump(
        ...
        while (true) {
            ...
            // get Labels!
            if (priorOpcode == Op_iaStore // IASTORE
                    && (opc == Op_biPush || opc == Op_siPush || (opc >= Op_iConst_0 && opc <= Op_iConst_5))
                    && (pNode.getType() == AbstractInsnNode.INT_INSN || pNode.getType() == AbstractInsnNode.INSN)) {

                int value = getOperand(workingClassName, pNode);
                if (debug) {
            ...
            if (pNode.getType() == AbstractInsnNode.INT_INSN && pNode.getOpcode() == Op_newArray) {
                ...
                int size = getOperand(workingClassName, pNode.getPrevious());
                stack.add(size);
                if (debug) {
                ...
                if (varRegisterNode.getType() == AbstractInsnNode.VAR_INSN) {
                    ...
                    register = -1 * getOperand(workingClassName, varRegisterNode);
                }
            ...
            if (size >= 0) {
                ...
                do {
                    ...
                    if (i == 0) {
                        ...
                        // insert short style
                        items.add(new InsnNode(Op_iConst_0 + i));
                        if (debug) {
                            System.out.println("Ref inserted for if = " + (Op_iConst_0 + i));
                        }

                    } else {
                        // insert long style
                        items.add(new IntInsnNode(Op_siPush, i));
                        if (debug) {
                    ...
                    if (i == 0) {
                        items.add(new JumpInsnNode(Op_ifne, nextIfLabel));
                    } else {
                        items.add(new JumpInsnNode(Op_if_icmpne, nextIfLabel)); // zero
                    }

                    items.add(new VarInsnNode(Op_aLoad, 0));
                    ...
                    if (arg >= 0 && arg <= 5) {
                        ...
                        items.add(new IntInsnNode(Op_siPush, arg));
                    }

                    items.add(new MethodInsnNode(Op_invokevirtual, workingClassName, "jump", "(I)V"));

                    if (!stack.isEmpty()) {
                        items.add(new JumpInsnNode(Op_goto, endIFLabel));
                    }
